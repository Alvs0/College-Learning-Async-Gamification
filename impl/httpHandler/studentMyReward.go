...
func StudentMyRewardHandler(generalData GeneralData, services impl.Services) http.HandlerFunc {
	return func(req *stdHttp.Request, res stdHttp.ResponseWriter) (interface{}, *http.ErrorWrapper) {
		var filepath = path.Join(getViewPath(), "student_my_reward.html")
		...
		getUserRewardReq := user.ListAndGetUserRewardReq{
			UserID:    getUserRes.Results[0].ID,
			CollegeID: getUserRes.Results[0].CollegeID,
		}

		var getUserRewardRes user.ListAndGetUserRewardRes
		if err := services.UserService.ListAndGetUserReward(getUserRewardReq, &getUserRewardRes); err != nil {
			return nil, http.WrapError(http.INTERNAL_SERVER_ERROR_CODE, err)
		}

		rewardBytes, err := json.Marshal(getUserRewardRes.UserReward.Rewards)
		if err != nil {
			return nil, http.WrapError(http.INTERNAL_SERVER_ERROR_CODE, err)
		}
